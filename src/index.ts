...
(async () => {
    ...
    while (1) {
        const { data } = await axios.get(process.env.leaderboardURI!);
        channel.sendTyping();
        const topThree = data.slice(3) as { username: string }[];
        const adapted = topThree.map(({ username }, idx) => `${idx + 1}: ${username}`);
